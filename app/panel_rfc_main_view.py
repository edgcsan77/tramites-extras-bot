...
<section class="hero"><div class="hero-top"><div><h1>Panel de Trámites Extras</h1><div class="hero-sub">CFE, IMSS y RENAPO · America/Monterrey</div></div><div class="toolbar"><a class="tool-link" href="/panel?token={_e(token)}&view=day">Hoy</a><a class="tool-link" href="/panel?token={_e(token)}&view=30d">30 días</a><a class="tool-link" href="/panel?token={_e(token)}&view=month">Mes actual</a><a class="tool-link" href="/panel?token={_e(token)}&view=prev_month">Mes anterior</a><a class="tool-link" href="/panel/bots?token={_e(token)}">Bots / mini paneles</a><a class="tool-link" href="/panel/providers?token={_e(token)}">Proveedores</a></div></div><div class="grid-hero"><div class="glass"><div class="section-title">Proveedores CFE</div><div class="provider-grid">{''.join(provider_cards)}</div></div><div class="glass"><div class="broadcast-header"><div><div class="section-title">Mensajes masivos</div><div class="helper">Estructura visual adaptada del panel RFC.</div></div><div style="min-width:240px"><label class="helper">Enviar a</label><select class="broadcast-select"><option>Todos</option><option>Bots activos</option><option>Grupos cliente</option></select></div></div><div class="broadcast-buttons"><button class="btn btn-success" type="button">Servicio activo</button><button class="btn btn-warning" type="button">Servicio restablecido</button><button class="btn btn-danger" type="button">Servicio suspendido</button></div><textarea class="textarea" placeholder="Escribe aquí el mensaje que deseas enviar..."></textarea><button class="btn btn-primary" style="margin-top:10px" type="button" onclick="alert('La difusión visual está lista; falta conectar el envío masivo de Extras.')">Enviar mensaje</button></div></div></section>
