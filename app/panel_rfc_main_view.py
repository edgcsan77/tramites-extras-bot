...
<section class="box"><div class="head"><strong>Filtros</strong><span class="small">Zona horaria: America/Monterrey</span></div><form method="get" class="filters"><input type="hidden" name="token" value="{_e(token)}"><input type="hidden" name="group_view" value="{_e(group_view)}"><select name="view"><option value="day" {selected_day}>Hoy</option><option value="30d" {selected_30d}>30 días</option><option value="month" {selected_month}>Mes actual</option><option value="prev_month" {selected_prev_month}>Mes anterior</option><option value="custom" {selected_custom}>Personalizado</option></select><input type="date" name="date_from" value="{_e(date_from)}"><input type="date" name="date_to" value="{_e(date_to)}"><input value="Todos los grupos" disabled><button class="btn btn-primary">Aplicar</button></form></section>
<section class="box"><div class="head"><strong>Vista de grupos</strong><span class="small">Consulta los grupos cliente y cambia la vista según su actividad.</span></div><div class="group-view-content"><a class="{group_view_class('all')}" href="/panel?token={_e(token)}&view={_e(view)}&date_from={_e(date_from)}&date_to={_e(date_to)}&group_view=all">Ver todos los grupos</a><a class="{group_view_class('activity')}" href="/panel?token={_e(token)}&view={_e(view)}&date_from={_e(date_from)}&date_to={_e(date_to)}&group_view=activity">Solo grupos con solicitudes del periodo</a></div></section><section class="box"><div class="head"><strong>Resumen por grupo cliente</strong><span class="small">{len(groups)} grupos</span></div><div class="table-wrap"><table><thead><tr><th>Grupo</th><th>Total</th><th>Entregadas</th><th>CFE</th><th>RENAPO</th><th>Bolsa CFE</th><th>Bolsa RENAPO</th><th>Estado</th><th>Actualización</th><th>Acciones</th></tr></thead><tbody>{''.join(group_rows) or '<tr><td colspan="10">Sin grupos.</td></tr>'}</tbody></table></div></section>
